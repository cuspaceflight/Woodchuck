...
static void usart_setup()
{
	...
	usart_set_stopbits(RADIO_USART, USART_STOPBITS_1);
...
{
	gpio_set_mode(RADIO_TX_GPIO, GPIO_MODE_OUTPUT_50_MHZ,
	              GPIO_CNF_OUTPUT_ALTFN_PUSHPULL, RADIO_TX_DATA);
...
/**
* Set up TIM6 for radio interrupts
...
static void setup_tim6()
{
	timer_reset(TIM6);
	/* 48MHz / 12kHz -1. */
	timer_set_prescaler(TIM6, 3999); /* 48MHz/12kHz - 1 */
...
/**
* Enable interrupts on TIM6 and start it.
...
static void start_tim6()
{
	...
	timer_enable_update_event(TIM6); /* Unclear */
...
/**
* Disable interrupts on TIM6 and stop it
* TODO Check that we don't need to reset the timer
*/
static void stop_tim6()
{
	timer_clear_flag(TIM6, TIM_SR_UIF);
...
/**
 * Interrupt service routine for TIM6 (Radio)
